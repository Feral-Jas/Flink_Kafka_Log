...
//        flink.find(new Document("name","dmjob"),new Document(map));
        sqljobs.find(new Document("_id", new ObjectId(args[0]))).forEach(
            (Consumer<? super Document>) item->{
                System.out.println(item.get("source"));
                System.out.println(item.get("sink"));
                System.out.println(item.get("sql"));
            }
        );
